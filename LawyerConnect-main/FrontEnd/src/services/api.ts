...
class ApiService {
  ...
  async sendAIChatMessage(message: string, chatId: string): Promise<any> {
    return this.request<any>('/ai-chat/message', {
      method: 'POST',
      body: JSON.stringify({ message, chatId }),
    });
  }

  async getAIChats(): Promise<any[]> {
    return this.request<any[]>('/ai-chat/chats', {
      method: 'GET',
    });
  }

  async getAIChatMessages(chatId: string): Promise<any[]> {
    return this.request<any[]>(`/ai-chat/chats/${chatId}/messages`, {
      method: 'GET',
    });
  }

  async deleteAIChat(chatId: string): Promise<void> {
    return this.request<void>(`/ai-chat/chats/${chatId}`, {
      method: 'DELETE',
    });
  }
  ...
  async uploadAIFile(file: File): Promise<any> {
    ...
    const url = `${this.baseUrl}/ai-chat/upload`;
    const token = this.getAuthToken();
  ...
  async getAIUploadedFiles(): Promise<any[]> {
    return this.request<any[]>('/ai-chat/files', {
      method: 'GET',
    });
  }

  async deleteAIFile(fileHash: string): Promise<void> {
    return this.request<void>(`/ai-chat/files/${fileHash}`, {
      method: 'DELETE',
    });
  }

  async checkAIHealth(): Promise<{ status: string }> {
    return this.request<{ status: string }>('/ai-chat/health', {
      method: 'GET',
    });
  }
